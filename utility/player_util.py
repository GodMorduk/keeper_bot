...
async def upload_skin(ctx, character, postfix, skin_message=None):
    if postfix.lower() == "null" or postfix == "":
        file_name = character
    else:
        ...
        async with session.get(m.attachments[0].url, allow_redirects=False) as r:
            if r.headers.get('content-type') == "image/png":
                if (int(r.headers.get('content-length')) / 1024) <= 150:  # 150 кб как можно догадаться
                    f = await aiofiles.open(f"{config_values.dir_skins}/{file_name}.png", mode='wb')
                    await f.write(await r.read())
                    await f.close()
                    await ctx.send("Скин успешно залит. Отличная работа (моя). Держи:\n"
                                   f"{config_values.link_skins}{file_name}.png")
                else:
                    await ctx.send(
...
async def get_skin_link(ctx, full_skin_name):
    ...
    for character in available_characters:
        if str(full_skin_name).startswith(character):
            if os.path.exists(f"{config_values.dir_skins}{full_skin_name}.png"):
                await ctx.send(f"Держи: {config_values.link_skins}{full_skin_name}.png")
            else:
...
async def get_skins_links(ctx, character):
    ...
    if character in available_characters:
        names = [pathlib.Path(x).stem for x in glob.glob(f"{config_values.dir_skins}{character}*")]
        if names:
            output = "Держи:\n"
            for character in names:
                output += f"{config_values.link_skins}{character}.png"
...
async def skins_eraser(ctx, full_skin_name):
    ...
    for character in available_characters:
        if str(full_skin_name).startswith(character):
            if os.path.exists(f"{config_values.dir_skins}/{full_skin_name}.png"):
                os.remove(f"{config_values.dir_skins}/{full_skin_name}.png")
